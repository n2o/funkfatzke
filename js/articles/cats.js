...
var addCat, articleItem, assignCats, catItem, catsInit, removeCat, root, selectedArticles, selectedCats, sqlGetArticles, sqlGetCats, sql_query, updateCatsOnArticleClick;
...
assignCats = function() {
  ...
  for (_i = 0, _len = selectedArticles.length; _i < _len; _i++) {
    ...
    if (count === selectedArticles.length) {
      query = query.slice(0, -1) + ";";
      callback = function() {
        return sql_query(query, "Zuordnung erfolgreich erstellt.", toggleInfo, true);
      };
...
sql_query = function(query, message, toggleInfo, toggle) {
  return $.ajax({
    type: "post",
